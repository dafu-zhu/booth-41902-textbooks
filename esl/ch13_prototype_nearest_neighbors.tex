...
\title{Chapter 13: Prototype Methods and Nearest-Neighbors}
...
\subsection*{13.2 Prototype Methods}
Throughout this chapter, our training data consists of the $N$ pairs $\left(x_{1}, g_{1}\right)$, $\ldots,\left(x_{n}, g_{N}\right)$ where $g_{i}$ is a class label taking values in $\{1,2, \ldots, K\}$. Prototype methods represent the training data by a set of points in feature space. These prototypes are typically not examples from the training sample, except in the case of 1-nearest-neighbor classification discussed later.
...
\begin{itemize}
  \item apply $K$-means clustering to the training data in each class separately, using $R$ prototypes per class;
...
\footnotetext{${ }^{1}$ The " $K$ " in $K$-means refers to the number of cluster centers. Since we have already reserved $K$ to denote the number of classes, we denote the number of clusters by $R$.
}K-means - 5 Prototypes per Class

\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-480_1539_746_202_418}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.1. Simulated example with three classes and five prototypes per class. The data in each class are generated from a mixture of Gaussians. In the upper panel, the prototypes were found by applying the $K$-means clustering algorithm separately in each class. In the lower panel, the LVQ algorithm (starting from the $K$-means solution) moves the prototypes away from the decision boundary. The broken purple curve in the background is the Bayes decision boundary.}
\end{center}
\end{figure}

\section*{Algorithm 13.1 Learning Vector Quantization-LVQ.}
\begin{enumerate}
  \item Choose $R$ initial prototypes for each class: $m_{1}(k), m_{2}(k), \ldots, m_{R}(k)$, $k=1,2, \ldots, K$, for example, by sampling $R$ training points at random from each class.
  \item Sample a training point $x_{i}$ randomly (with replacement), and let $(j, k)$ index the closest prototype $m_{j}(k)$ to $x_{i}$.\\
(a) If $g_{i}=k$ (i.e., they are in the same class), move the prototype towards the training point:
\end{enumerate}

$$
m_{j}(k) \leftarrow m_{j}(k)+\epsilon\left(x_{i}-m_{j}(k)\right),
$$
...
\begin{enumerate}
  \setcounter{enumi}{2}
  \item Repeat step 2, decreasing the learning rate $\epsilon$ with each iteration towards zero.\\
...
\subsection*{13.2.2 Learning Vector Quantization}
...
\subsection*{13.2.3 Gaussian Mixtures}
...
Similarly, when Gaussian mixture models are used to represent the feature density in each class, it produces smooth posterior probabilities $\hat{p}(x)= \left\{\hat{p}_{1}(x), \ldots, \hat{p}_{K}(x)\right\}$ for classifying $x$ (see (12.60) on page 449.) Often this is interpreted as a soft classification, while in fact the classification rule is $\hat{G}(x)=\arg \max _{k} \hat{p}_{k}(x)$. Figure 13.2 compares the results of $K$-means and Gaussian mixtures on the simulated mixture problem of Chapter 2. We see that although the decision boundaries are roughly similar, those for the mixture model are smoother (although the prototypes are in approximately the same positions.) We also see that while both procedures devote a blue prototype (incorrectly) to a region in the northwest, the Gaussian mixture classifier can ultimately ignore this region, while $K$-means cannot. LVQ gave very similar results to $K$-means on this example, and is not shown.

\section*{$13.3 k$-Nearest-Neighbor Classifiers}
These classifiers are memory-based, and require no model to be fit. Given a query point $x_{0}$, we find the $k$ training points $x_{(r)}, r=1, \ldots, k$ closest in distance to $x_{0}$, and then classify using majority vote among the $k$ neighbors.

\section*{K-means - 5 Prototypes per Class}
\begin{center}
\includegraphics[max width=\textwidth, alt={}]{3c826f12-a852-4cfa-af69-8d4caa44274f-483_688_739_205_423}
\end{center}

\begin{figure}[h]
\begin{center}
\captionsetup{labelformat=empty}
\caption{Gaussian Mixtures - 5 Subclasses per Class}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-483_684_738_1051_423}
\end{center}
\end{figure}
...
\begin{equation*}
d_{(i)}=\left\|x_{(i)}-x_{0}\right\| . \tag{13.1}
\end{equation*}
...
We now give more detail for misclassification loss. At $x$ let $k^{*}$ be the dominant class, and $p_{k}(x)$ the true conditional probability for class $k$. Then


\begin{align*}
\text { Bayes error } & =1-p_{k^{*}}(x)  \tag{13.2}\\
\text { 1-nearest-neighbor error } & =\sum_{k=1}^{K} p_{k}(x)\left(1-p_{k}(x)\right)  \tag{13.3}\\
...
\begin{center}
\captionsetup{labelformat=empty}
\caption{15-Nearest Neighbors}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-485_1541_746_205_418}
\end{center}
\end{figure}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-486_1470_811_151_353}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.4. $k$-nearest-neighbors on the two-class mixture data. The upper panel shows the misclassification errors as a function of neighborhood size. Standard error bars are included for 10-fold cross validation. The lower panel shows the decision boundary for 7-nearest-neighbors, which appears to be optimal for minimizing test error. The broken purple curve in the background is the Bayes decision boundary.}
\end{center}
\end{figure}

ties $p_{k}(x), k=1, \ldots, K$. For $K=2$ the 1 -nearest-neighbor error rate is $2 p_{k^{*}}(x)\left(1-p_{k^{*}}(x)\right) \leq 2\left(1-p_{k^{*}}(x)\right)$ (twice the Bayes error rate). More generally, one can show (Exercise 13.3)


\begin{equation*}
\sum_{k=1}^{K} p_{k}(x)\left(1-p_{k}(x)\right) \leq 2\left(1-p_{k^{*}}(x)\right)-\frac{K}{K-1}\left(1-p_{k^{*}}(x)\right)^{2} . \tag{13.5}
\end{equation*}
...
\subsection*{13.3.1 Example: A Comparative Study}
We tested the nearest-neighbors, $K$-means and LVQ classifiers on two simulated problems. There are 10 independent features $X_{j}$, each uniformly distributed on $[0,1]$. The two-class 0-1 target variable is defined as follows:


\begin{align*}
& Y=I\left(X_{1}>\frac{1}{2}\right) ; \quad \text { problem 1: "easy", } \\
& Y=I\left(\operatorname{sign}\left\{\prod_{j=1}^{3}\left(X_{j}-\frac{1}{2}\right)\right\}>0\right) ; \quad \text { problem 2: "difficult." } \tag{13.6}
\end{align*}


Hence in the first problem the two classes are separated by the hyperplane $X_{1}=1 / 2$; in the second problem, the two classes form a checkerboard pattern in the hypercube defined by the first three features. The Bayes error rate is zero in both problems. There were 100 training and 1000 test observations.
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-488_1107_1098_437_212}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.5. Mean ± one standard error of misclassification error for near-est-neighbors, $K$-means (blue) and LVQ (red) over ten realizations for two simulated problems: "easy" and "difficult," described in the text.}
\end{center}
\end{figure}

\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-489_741_1130_128_210}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.6. The first four panels are LANDSAT images for an agricultural area in four spectral bands, depicted by heatmap shading. The remaining two panels give the actual land usage (color coded) and the predicted land usage using a five-nearest-neighbor rule described in the text.}
\end{center}
\end{figure}
...
\subsection*{13.3.2 Example: $k$-Nearest-Neighbors and Image Scene Classification}
...
\end{tabular}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.7. A pixel and its 8-neighbor feature map.}
\end{center}
\end{table}
...
\subsection*{13.3.3 Invariant Metrics and Tangent Distance}
...
We wish to remove the effect of rotation in measuring distances between two digits of the same class. Consider the set of pixel values consisting of the original " 3 " and its rotated versions. This is a one-dimensional curve in $\mathbb{R}^{256}$, depicted by the green curve passing through the "3" in Figure 13.10. Figure 13.11 shows a stylized version of $\mathbb{R}^{256}$, with two images indicated by $x_{i}$ and $x_{i^{\prime}}$. These might be two different " 3 "s, for example. Through each image we have drawn the curve of rotated versions of that image, called

\section*{STATLOG results}
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-491_717_1073_333_214}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.8. Test-error performance for a number of classifiers, as reported by the STATLOG project. The entry DANN is a variant of $k$-nearest neighbors, using an adaptive metric (Section 13.4.2).}
\end{center}
\end{figure}

\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-491_396_1121_1441_212}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.9. Examples of grayscale images of handwritten digits.}
\end{center}
\end{figure}

\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-492_755_685_133_432}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.10. The top row shows a " 3 " in its original orientation (middle) and rotated versions of it. The green curve in the middle of the figure depicts this set of rotated " 3 " in 256 -dimensional space. The red line is the tangent line to the curve at the original image, with some " 3 " s on this tangent line, and its equation shown at the bottom of the figure.}
\end{center}
\end{figure}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-493_682_602_131_492}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.11. Tangent distance computation for two images $x_{i}$ and $x_{i^{\prime}}$. Rather than using the Euclidean distance between $x_{i}$ and $x_{i^{\prime}}$, or the shortest distance between the two curves, we use the shortest distance between the two tangent lines.}
\end{center}
\end{figure}
...
\begin{center}
\captionsetup{labelformat=empty}
\caption{TABLE 13.1. Test error rates for the handwritten ZIP code problem.}
\begin{tabular}{lr}
\hline
Method & Error rate \\
\hline
...
\subsection*{13.4 Adaptive Nearest-Neighbor Methods}
...
\begin{equation*}
\operatorname{median}(R)=v_{p}^{-1 / p}\left(1-\frac{1}{2}^{1 / N}\right)^{1 / p}, \tag{13.7}
\end{equation*}


where $v_{p} r^{p}$ is the volume of the sphere of radius $r$ in $p$ dimensions. Figure 13.12 shows the median radius for various training sample sizes and dimensions. We see that median radius quickly approaches 0.5 , the distance to the edge of the cube.
...
The discriminant adaptive nearest-neighbor (DANN) metric at a query point $x_{0}$ is defined by


\begin{equation*}
D\left(x, x_{0}\right)=\left(x-x_{0}\right)^{T} \boldsymbol{\Sigma}\left(x-x_{0}\right), \tag{13.8}
\end{equation*}
...
\begin{align*}
\boldsymbol{\Sigma} & =\mathbf{W}^{-1 / 2}\left[\mathbf{W}^{-1 / 2} \mathbf{B} \mathbf{W}^{-1 / 2}+\epsilon \mathbf{I}\right] \mathbf{W}^{-1 / 2} \\
& =\mathbf{W}^{-1 / 2}\left[\mathbf{B}^{*}+\epsilon \mathbf{I}\right] \mathbf{W}^{-1 / 2} \tag{13.9}
\end{align*}


Here $\mathbf{W}$ is the pooled within-class covariance matrix $\sum_{k=1}^{K} \pi_{k} \mathbf{W}_{k}$ and $\mathbf{B}$ is the between class covariance matrix $\sum_{k=1}^{K} \pi_{k}\left(\bar{x}_{k}-\bar{x}\right)\left(\bar{x}_{k}-\bar{x}\right)^{T}$, with W and $\mathbf{B}$ computed using only the 50 nearest neighbors around $x_{0}$. After computation of the metric, it is used in a nearest-neighbor rule at $x_{0}$.
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{3c826f12-a852-4cfa-af69-8d4caa44274f-498_512_771_137_379}
\captionsetup{labelformat=empty}
\caption{FIGURE 13.15. Ten-dimensional simulated example: boxplots of the test error rates over ten realizations, for standard 5-nearest-neighbors, LVQ with 50 centers, and discriminant-adaptive 5-nearest-neighbors}
\end{center}
\end{figure}

\subsection*{13.4.2 Global Dimension Reduction for Nearest-Neighbors}
The discriminant-adaptive nearest-neighbor method carries out local dimension reduction-that is, dimension reduction separately at each query point. In many problems we can also benefit from global dimension reduction, that is, apply a nearest-neighbor rule in some optimally chosen subspace of the original feature space. For example, suppose that the two classes form two nested spheres in four dimensions of feature space, and there are an additional six noise features whose distribution is independent of class. Then we would like to discover the important four-dimensional subspace, and carry out nearest-neighbor classification in that reduced subspace. Hastie and Tibshirani (1996a) discuss a variation of the discriminantadaptive nearest-neighbor method for this purpose. At each training point $x_{i}$, the between-centroids sum of squares matrix $\mathbf{B}_{i}$ is computed, and then these matrices are averaged over all training points:


\begin{equation*}
\overline{\mathbf{B}}=\frac{1}{N} \sum_{i=1}^{N} \mathbf{B}_{i} \tag{13.10}
\end{equation*}


Let $e_{1}, e_{2}, \ldots, e_{p}$ be the eigenvectors of the matrix $\overline{\mathbf{B}}$, ordered from largest to smallest eigenvalue $\theta_{k}$. Then these eigenvectors span the optimal subspaces for global subspace reduction. The derivation is based on the fact that the best rank- $L$ approximation to $\overline{\mathbf{B}}, \overline{\mathbf{B}}_{[L]}=\sum_{\ell=1}^{L} \theta_{\ell} e_{\ell} e_{\ell}^{T}$, solves the least squares problem


\begin{equation*}
\min _{\operatorname{rank}(\mathbf{M})=L} \sum_{i=1}^{N} \operatorname{trace}\left[\left(\mathbf{B}_{i}-\mathbf{M}\right)^{2}\right] . \tag{13.11}
\end{equation*}


Since each $\mathbf{B}_{i}$ contains information on (a) the local discriminant subspace, and (b) the strength of discrimination in that subspace, (13.11) can be seen\\
...
\subsection*{13.5 Computational Considerations}
...
\section*{Exercises}
Ex. 13.1 Consider a Gaussian mixture model where the covariance matrices are assumed to be scalar: $\boldsymbol{\Sigma}_{r}=\sigma \mathbf{I} \forall r=1, \ldots, R$, and $\sigma$ is a fixed parameter. Discuss the analogy between the $K$-means clustering algorithm and the EM algorithm for fitting this mixture model in detail. Show that in the limit $\sigma \rightarrow 0$ the two methods coincide.

Ex. 13.2 Derive formula (13.7) for the median radius of the 1 -nearestneighborhood.

Ex. 13.3 Let $E^{*}$ be the error rate of the Bayes rule in a $K$-class problem, where the true class probabilities are given by $p_{k}(x), k=1, \ldots, K$. Assuming the test point and training point have identical features $x$, prove (13.5)

$$
\sum_{k=1}^{K} p_{k}(x)\left(1-p_{k}(x)\right) \leq 2\left(1-p_{k^{*}}(x)\right)-\frac{K}{K-1}\left(1-p_{k^{*}}(x)\right)^{2} .
$$

where $k^{*}=\arg \max _{k} p_{k}(x)$. Hence argue that the error rate of the 1 -nearest-neighbor rule converges in $L_{1}$, as the size of the training set increases, to a value $E_{1}$, bounded above by
...
Ex. 13.4 Consider an image to be a function $F(x): \mathbb{R}^{2} \mapsto \mathbb{R}^{1}$ over the twodimensional spatial domain (paper coordinates). Then $F\left(c+x_{0}+\mathbf{A}\left(x-x_{0}\right)\right)$ represents an affine transformation of the image $F$, where $\mathbf{A}$ is a $2 \times 2$ matrix.

\begin{enumerate}
  \item Decompose A (via Q-R) in such a way that parameters identifying the four affine transformations (two scale, shear and rotation) are clearly identified.
  \item Using the chain rule, show that the derivative of $F\left(c+x_{0}+\mathbf{A}\left(x-x_{0}\right)\right)$ w.r.t. each of these parameters can be represented in terms of the two spatial derivatives of $F$.
  \item Using a two-dimensional kernel smoother (Chapter 6), describe how to implement this procedure when the images are quantized to $16 \times 16$ pixels.
\end{enumerate}

Ex. 13.5 Let $\mathbf{B}_{i}, i=1,2, \ldots, N$ be square $p \times p$ positive semi-definite matrices and let $\overline{\mathbf{B}}=(1 / N) \sum \mathbf{B}_{i}$. Write the eigen-decomposition of $\overline{\mathbf{B}}$ as $\sum_{\ell=1}^{p} \theta_{\ell} e_{\ell} e_{\ell}^{T}$ with $\theta_{\ell} \geq \theta_{\ell-1} \geq \cdots \geq \theta_{1}$. Show that the best rank- $L$ approximation for the $\mathbf{B}_{i}$,

$$
\min _{\operatorname{rank}(\mathbf{M})=L} \sum_{i=1}^{N} \operatorname{trace}\left[\left(\mathbf{B}_{i}-\mathbf{M}\right)^{2}\right],
$$

is given by $\overline{\mathbf{B}}_{[L]}=\sum_{\ell=1}^{L} \theta_{\ell} e_{\ell} e_{\ell}^{T}$. (Hint: Write $\sum_{i=1}^{N} \operatorname{trace}\left[\left(\mathbf{B}_{i}-\mathbf{M}\right)^{2}\right]$ as

$$
\left.\sum_{i=1}^{N} \operatorname{trace}\left[\left(\mathbf{B}_{i}-\overline{\mathbf{B}}\right)^{2}\right]+\sum_{i=1}^{N} \operatorname{trace}\left[(\mathbf{M}-\overline{\mathbf{B}})^{2}\right]\right)
$$

Ex. 13.6 Here we consider the problem of shape averaging. In particular, $\mathbf{L}_{i}, i=1, \ldots, M$ are each $N \times 2$ matrices of points in $\mathbb{R}^{2}$, each sampled from corresponding positions of handwritten (cursive) letters. We seek an affine invariant average $\mathbf{V}$, also $N \times 2, \mathbf{V}^{T} \mathbf{V}=I$, of the $M$ letters $\mathbf{L}_{i}$ with the following property: $\mathbf{V}$ minimizes
...
\begin{enumerate}
  \item Replicate the results in the left panel of Figure 13.5.
  \item Estimate the misclassification errors using fivefold cross-validation, and compare the error rate curves to those in 1 .
  \item Consider an "AIC-like" penalization of the training set misclassification error. Specifically, add $2 t / N$ to the training set misclassification error, where $t$ is the approximate number of parameters $N / r, r$ being the number of nearest-neighbors. Compare plots of the resulting penalized misclassification error to those in 1 and 2 . Which method gives a better estimate of the optimal number of nearest-neighbors: cross-validation or AIC?
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Nearest-neighbors with hints: ten randomly rotated versions of each data point are added to the training set before applying nearestneighbors.
  \item Invariant metric nearest-neighbors, using Euclidean distance invariant to rotations about the origin.
  \item Tangent distance nearest-neighbors.
\end{enumerate}
